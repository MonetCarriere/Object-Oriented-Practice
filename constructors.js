...
function it(description, contents){
  ...
  // SHOULD MAKE SADIE HAPPY WHEN MASON PETS HER
  ...
  //SHOULD MAKE SADIE BLACK
  Dog.prototype = function(sadie){
  this.color
  }


//SHOULD MAKE MOONSHINE HUNGRY AND SADIE NOT HUNGRY

Dog.prototype = function() {
  console.log(moonshine.hungry= true; && sadie.hungry= false)
}


//SHOULD MAKE MOONSHINE NO LONGER HUNGRY WHEN YOU FEED HIM

Dog.prototype = function expect(target) {
    toBe: function(expectation) {
      if (julia.feed(moonshine) === expectation) {
        console.log(moonshine no longer hungry);
        return true;
      } else {
        console.log(moonshine is hungry);
        return false;
      }
...
//SHOULD NOT AFFECT ATTICUS AND MOONSHINE'S OWNER PROPERTIES WHEN SETTING MASON AS SADIE'S OWNER

Dog.prototype = function expect(target) {
expect(moonshine.owner).toBe(undefined);
    expect(atticus.owner).toBe(undefined);
      expect(mason.owner).toBe(sadie);
        console.log(mason.owner);
}

//SHOULD MAKE JULIA COOL AND MASON NOT COOL

Human.prototype = function() {
 console.log (julia.cool = true && mason.cool = false)
}
